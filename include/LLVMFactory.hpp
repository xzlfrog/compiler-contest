...
#include"LocalMemoryOperation.hpp"
#include"TypeConversionOperation.hpp"
...
public:
    ...
    //func必须来自于一个之前已经定义/声明过的函数对应的FuncSymbol*
    template<typename ...Args>
    static CallLLVM* createCallLLVM(FuncSymbol* func,Args... args){
        CallLLVM* callLLVM=new CallLLVM();
        callLLVM->addArguments(...args);
    ...
    static GetElementPtrLLVM* createGetElementPtrLLVM(ArraySymbol* ptrval,std::vector<dataType>type,std::vector<BasicSymbol*>idx);
};
