...
Expression*  create_binary_expr(int op, Expression* exp1,Expression* exp2){
    ...
    // if(exp1->sym->getDataType() != exp2->sym->getDataType()) {
    //     fprintf(stderr, "error in [frontendTool.cpp]\n两个表达式的数据类型不匹配哦: %s, %s\n", 
    //             exp1->sym->getDataType().toString().c_str(), 
    //             exp2->sym->getDataType().toString().c_str());
    //     exit(1);
    // }

    // if(exp1->sym->getType() != symType::variable || exp2->sym->getType() != symType::variable) {
    //     fprintf(stderr, "error in [frontendTool.cpp]\n两个表达式必须是变量哦: %s, %s\n", 
    //             exp1->sym->getType(), 
    //             exp2->sym->getType());
    //     exit(1);
    ...
    if(exp1->sym->getDataType() == dataType::i32 && exp2->sym->getDataType() == dataType::i32) {
    ...
    case '-':   
        ...
        break;  
    case '*':   
        llvm_type = LLVMtype::mul;
        break;
    case '/':   
        llvm_type = LLVMtype::sdiv;
        break;
    case '%':
        llvm_type = LLVMtype::srem;
        break;
    case LT:
        llvm_type = LLVMtype::icmp_slt;
        break;
    case GT:
        llvm_type = LLVMtype::icmp_sgt;
        break;
    case LE:
        llvm_type = LLVMtype::icmp_sle;
        break;
    case GE:
        llvm_type = LLVMtype::icmp_sge;
        break;
    case EQ:
        llvm_type = LLVMtype::icmp_eq;
        break;
    case NE:
        llvm_type = LLVMtype::icmp_ne;
        break;
    }
}
    else if(exp1->sym->getDataType() == dataType::f32 && exp2->sym->getDataType() == dataType::f32) {
        switch (op):
        {
        case '+':
        ...
        case '-':   
            ...
            break;  
        case '*':   
            llvm_type = LLVMtype::llvm_fmul;
            break;
        case '/':   
            llvm_type = LLVMtype::llvm_fdiv;
            break;
        case '%':
            llvm_type = LLVMtype::llvm_frem;
            break;
        case LT:
            llvm_type = LLVMtype::fcmp_olt;
            break;
        case GT:
            llvm_type = LLVMtype::fcmp_ogt;
            break;
        case LE:
            llvm_type = LLVMtype::fcmp_ole;
            break;
        case GE:
            llvm_type = LLVMtype::fcmp_oge;
            break;
        case EQ:
            llvm_type = LLVMtype::fcmp_oeq;
            break;
        case NE:
            llvm_type = LLVMtype::fcmp_one;
            break;
        }
    } 
    ...
    VarSymbol* tmpRes = SymbolFactory::createTmpVarSymbolWithScope(dataType::exp1->sym->getDataType(), scope);
    
    Res->sym = tmpRes;

    ArithmeticOperationLLVM* operation = LLVMfactory::createBasicOperationLLVM(llvm_type, tmpRes, exp1->sym, exp2->sym);
    Res->llvm = operation;

    LLVMList::InsertAfter(exp1->llvm, exp2->llvm);
    LLVMList::InsertAfter(exp2->llvm, operation);
